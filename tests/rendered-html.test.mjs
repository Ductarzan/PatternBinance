...
test("keeps market logic deterministic and Gemini server-only", async () => {
  ...
  assert.match(watchlistRoute, /RSI_OVERBOUGHT_THRESHOLD = 90/);
  assert.match(watchlistRoute, /DIVERGENCE_LOOKBACK = 80/);
  assert.match(watchlistRoute, /findRsiDivergence/);
  ...
  assert.match(watchlistRoute, /value > RSI_OVERBOUGHT_THRESHOLD/);
  assert.match(watchlistRoute, /bao gồm nến đang chạy/);
  assert.match(watchlistRoute, /bullishDivergences\.length/);
  assert.match(watchlistRoute, /bearishDivergences\.length/);
