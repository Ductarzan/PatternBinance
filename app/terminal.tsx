          ...
          <section className="watchlist-panel panel">
            <div className="panel-header watchlist-header">
              <div className="watchlist-title">
                ...
                <div><span className="eyebrow">RSI OVERSOLD SCANNER</span><h2>Top coin RSI(7) &lt; 20</h2></div>
              </div>
              <div className="watchlist-actions">
                {watchlist
                  ...
                  : <span>Đang tìm RSI(7) &lt; 20 trong top 200 volume</span>}
                <button
                  type="button"
            ...
            {watchlistError && !watchlist ? (
              ...
              <div className={watchlistLoading ? "watchlist-cards loading" : "watchlist-cards"}>
                {watchlist ? watchlist.items.slice(0, 6).map((item, index) => {
                  ...
                  const itemClass = divergence ? "positive" : "neutral";
                  return (
                    <button
                      type="button"
                      ...
                      aria-label={divergence
                        ? `Phân tích ${item.ticker}, tín hiệu Long do phân kỳ tăng ${divergence.frame}`
                        : `Phân tích ${item.ticker}, RSI(7) thấp nhất ${item.lowestRsi}`}
                    >
            ...
            <div className="watchlist-foot">
              <span><CircleDot size={11} /> {watchlist?.methodology || "RSI(7) < 20 là bắt buộc; coin có phân kỳ tăng được gắn nhãn LONG."} Tự quét lại mỗi 90 giây.</span>
              ...
              <div className={watchlistLoading ? "watchlist-cards loading" : "watchlist-cards"}>
                {watchlist ? watchlist.overboughtItems.slice(0, 6).map((item, index) => {
                  ...
                  return (
                    ...
                    onClick={() => selectSymbol(item.symbol)}
                    aria-label={divergence
                      ? `Phân tích ${item.ticker}, tín hiệu Short do phân kỳ giảm ${divergence.frame}`
                      : `Phân tích ${item.ticker}, RSI(7) cao nhất ${item.highestRsi}`}
                    >
                      <span className="watch-rank">#{index + 1}</span>
